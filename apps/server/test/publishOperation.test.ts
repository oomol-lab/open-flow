...
it('keeps Live fixed while persistent Publish work is pending or failed and activates one Publication after recovery', async () => {
  ...
  const first = await service.control.publishFlow('operator', flowId, firstRevisionId, 'open-flow-engine/v3', null, 'publish-first')
  ...
  const failed = await service.control.publishFlow('operator', flowId, failedRevisionId, 'open-flow-engine/v3', firstPublicationId, 'publish-failed')
  const replay = await service.control.publishFlow('operator', flowId, failedRevisionId, 'open-flow-engine/v3', firstPublicationId, 'publish-failed')
  expect(replay).toEqual(failed)
  await expect(
    service.control.publishFlow('operator', flowId, failedRevisionId, 'open-flow-engine/v3', firstPublicationId, 'publish-concurrent'),
  ...
  const ready = await service.control.publishFlow('operator', flowId, readyRevisionId, 'open-flow-engine/v3', firstPublicationId, 'publish-ready')
