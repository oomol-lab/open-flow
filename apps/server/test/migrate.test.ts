...
it('applies the Flow-first schema without foreign keys', async () => {
  ...
  try {
    expect(version(database)).toBe(11)
...
it('rejects a newer Flow schema version without modifying it', async () => {
  ...
  expect(() => migrateDatabase(file)).toThrow('SQLite schema version 12 is newer than the supported version 11.')
