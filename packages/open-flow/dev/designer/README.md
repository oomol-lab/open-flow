...
`?story=diagnostics-panel` 展示发布控件旁的问题入口和按节点定位的白色诊断面板，包括同一节点的多条问题及无法归属节点的问题。
...
- **Canvas operations** (`?story=canvas-history`)：交互画布支持混合删除恢复、复制粘贴、移动、代码编辑后保留画布撤销、快捷键和文本焦点隔离；说明栏的 Hold saves / Release saves 控制模拟传输。
