...
test('keeps shared popup motion accessible without replacing Designer layout ownership', async () => {
  ...
  assert.match(designerSelect, /className=\{styles\.menu\}/)
  assert.match(designerSelect, /container=\{popupContainer\}/)
...
test('keeps responsive control density on component APIs', async () => {
  ...
  assert.equal((workspaceHeader.match(/size="sm"/g) ?? []).length, 2)
  assert.doesNotMatch(workspaceHeader, /className="validation-state"[\s\S]*?size="sm"/)
  assert.doesNotMatch(workspaceHeader, /onClick=\{onRunDraft\}[\s\S]*?size="sm"/)
  assert.doesNotMatch(workspaceHeader, /onClick=\{onRunLive\}[\s\S]*?size="sm"/)
  assert.doesNotMatch(workspaceHeader, /store\.publications\.publish\(\)[\s\S]*?size="sm"/)
...
test('keeps responsive overlays aligned with the Workbench container and keyboard state', async () => {
  ...
  assert.match(workspaceStyles, /\.diagnostics-panel \{[\s\S]*?top: 64px;/)
  assert.match(workspaceStyles, /\.run-input-panel \{[\s\S]*?top: 64px;/)
