...
export function conditionBranchSummary(node: Omit<FlowCanvasViewConditionNode, 'position'>, output: string, t: TFunction): string {
  ...
  if (item != null) {
    ...
    return (node.diagnostics ?? 0) > 0 || node.run?.status === 'error' ? t('condition.invalidCase') : conditionCaseSummary(item, t)
  }
...
export function nodeCardContent(node: NodeContent) {
  ...
  const collapsible = node.kind != 'condition' && (values.length > 0 || !!schedules?.length || images.length > 0 || (!inline && !!summary))
  return { values, summary, schedules, images, inline, collapsible, hidden: collapsible && node.contentHidden === true }
}
