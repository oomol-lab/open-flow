...
import { DEFAULT_POSITION } from './canvas.ts'
...
export function toRFNodeId(nodeId: NodeId, nodeType?: NodeType): RFNodeId {
  const prefix = nodeType === NODE_TYPE.CommentNode ? 'c:' : 'm:'
...
export function applyEdgeChanges<TRFEdge extends RFEdge = RFEdge>(
  ...
  editable$: ReadonlyVal<boolean | undefined>,
): Set<FlowCanvasViewEdge> | undefined {
  let toRemoveConnections: Set<FlowCanvasViewEdge> | undefined
