...
import type { NodeContent } from '../../graph/FlowCanvas/nodeContent.ts'
...
function createTestSetup(connections = val<readonly FlowCanvasViewEdge[]>([]), autoLayout = true): TestSetup {
...
describe('CanvasStore graph projection', () => {
...
describe('CanvasStore layout', () => {
  ...
  it('leaves enough horizontal space for edge turns between connected nodes', async () => {
    ...
    const setup = createTestSetup(
      ...
      ] as readonly FlowCanvasViewEdge[]),
    )
