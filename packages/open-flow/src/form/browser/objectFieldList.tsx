          ...
          {children(
            ...
            sorting && onReorder && (
              <Button
                ...
                aria-label={t('valueEditor.reorder', { name })}
                ...
                onPointerMove={(event) => {
                  ...
                  setActive(name)
                  const row = document.elementFromPoint(event.clientX, event.clientY)?.closest<HTMLElement>('[data-object-field]')
                  const target = row?.parentElement === list.current ? names.indexOf(row.dataset.objectField!) : -1
