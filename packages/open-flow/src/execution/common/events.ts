...
const nodeKinds = new Set(['agent', 'approval', 'condition', 'connector', 'javascript', 'llm', 'subflow', 'value', 'wait'])
...
function nodeKind(value: unknown): 'agent' | 'approval' | 'condition' | 'connector' | 'javascript' | 'llm' | 'subflow' | 'value' | 'wait' | undefined {
  if (value === undefined) return
  if (!nodeKinds.has(value as string)) {
    throw new TypeError('Runtime node.started nodeKind is invalid.')
  }
  return value as 'agent' | 'approval' | 'condition' | 'connector' | 'javascript' | 'llm' | 'subflow' | 'value' | 'wait'
}
