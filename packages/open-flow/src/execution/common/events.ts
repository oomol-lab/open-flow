...
const nodeKinds = new Set(['agent', 'condition', 'connector', 'javascript', 'llm', 'subflow', 'value', 'wait'])
...
function nodeKind(value: unknown): 'agent' | 'condition' | 'connector' | 'javascript' | 'llm' | 'subflow' | 'value' | 'wait' | undefined {
  if (value === undefined) return
  if (!nodeKinds.has(value as string)) {
    throw new TypeError('Runtime node.started nodeKind is invalid.')
  }
  return value as 'agent' | 'condition' | 'connector' | 'javascript' | 'llm' | 'subflow' | 'value' | 'wait'
}
