  ...
  it('starts and reads one Publish operation', async () => {
    ...
    expect(request).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        body: JSON.stringify({ engineContract: 'open-flow-engine/v5', expectedLivePublicationId: null, version: 1 }),
        method: 'POST',
      }),
    )
...
const editor = {
  ...
  draft: {
    ...
    revisionId: flow.draftRevisionId,
    version: 1,
    content: { document: { bindings: {}, graph: { edges: [], nodes: {} }, subflows: {}, tasks: {} }, modules: {}, modelVersion: 2 },
  },
