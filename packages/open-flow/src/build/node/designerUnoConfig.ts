  ...
  presets: [
    presetIcons({
      ...
      collections: {
        ...
        'oomol': async () => {
          const collections = JSON.parse(await readFile(path.join(sourceRoot, 'src/ui/browser/icons/extra-icon.json'), 'utf8')) as Record<string, IconifyJSON>
          return collections.oomol!
        },
        'file': fileIcons,
        'custom': {
