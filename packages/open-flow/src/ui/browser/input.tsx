...
const Input = React.forwardRef<HTMLInputElement, React.ComponentPropsWithoutRef<'input'> & { controlSize?: 'default' | 'field' }>(function Input(
  { className, type, controlSize = 'default', ...props },
  ref,
) {
  return (
    <InputPrimitive
      ...
      className={cn(
        ...
        controlSize === 'field' && 'h-[30px] px-2 text-xs md:text-xs',
        className,
      )}
      {...props}
    />
