...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        "flex w-fit items-center justify-between gap-1.5 rounded-[var(--ui-control-radius,calc(var(--ui-radius)_+_2px))] border border-input bg-[var(--ui-control-background,var(--ui-muted))] py-2 pr-2 pl-2.5 text-sm whitespace-nowrap aria-expanded:bg-muted transition-colors outline-none select-none focus-visible:outline-2 focus-visible:outline-solid focus-visible:ring-0 focus-visible:aria-invalid:ring-0 focus-visible:outline-offset-1 focus-visible:outline-ring focus-visible:aria-invalid:outline-destructive disabled:cursor-[var(--ui-disabled-cursor,not-allowed)] disabled:opacity-[var(--ui-disabled-opacity,0.5)] aria-invalid:border-destructive data-placeholder:text-muted-foreground data-[size=default]:h-8 data-[size=field]:h-[30px] data-[size=sm]:h-7 data-[size=sm]:rounded-[var(--ui-control-radius,var(--ui-radius))] *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 dark:bg-[var(--ui-control-background,var(--ui-muted))] [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        variant == 'subtle' &&
          'rounded-[var(--ui-control-radius,calc(var(--ui-radius)_-_2px))] border-transparent bg-[var(--ui-control-background,var(--ui-muted))] px-2 py-0.5 text-xs data-[size=sm]:h-6 data-[size=sm]:rounded-[var(--ui-control-radius,calc(var(--ui-radius)_-_2px))] dark:bg-[var(--ui-control-background,var(--ui-muted))]',
        className,
      )}
      {...props}
    >
...
function SelectContent({
  ...
  return (
    <SelectPrimitive.Portal container={container} className="contents">
...
function SelectItem({ className, children, ...props }: SelectPrimitive.Item.Props) {
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-1.5 rounded-[var(--ui-control-radius,var(--ui-radius))] py-1 pr-8 pl-1.5 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-[var(--ui-disabled-opacity,0.5)] [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className,
      )}
      {...props}
    >
