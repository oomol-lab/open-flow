...
function DialogOverlay({ className, ...props }: DialogPrimitive.Backdrop.Props) {
  return (
    <DialogPrimitive.Backdrop
      ...
      className={cn(
        'fixed inset-0 isolate z-50 bg-black/10 supports-backdrop-filter:backdrop-blur-xs transition-opacity duration-150 ease-out data-starting-style:opacity-0 data-ending-style:opacity-0 motion-reduce:transition-none',
        className,
      )}
      {...props}
    />
    ...
    <DialogPortal container={container}>
      ...
      >
        ...
        {showCloseButton && (
          <Tooltip>
            <TooltipTrigger
              render={<DialogPrimitive.Close data-slot="dialog-close" render={<Button className="absolute top-2 right-2" size="icon-sm" variant="ghost" />} />}
            >
              <XIcon />
              <span className="sr-only">{closeLabel}</span>
            </TooltipTrigger>
            <TooltipContent container={container}>{closeLabel}</TooltipContent>
          </Tooltip>
        )}
      </DialogPrimitive.Popup>
    </DialogPortal>
