...
每个 GitHub Release 都会发布其 tag。稳定 Release 会同时移动 `latest`；pre-release 不会，因此 `latest` 不会指向 beta。

Open Flow 目前处于 beta 阶段: `latest` 会随首个稳定 Release 出现，在此之前请使用 beta Release tag，例如 `v0.1.0-beta.1`。生产环境请固定到 Release tag，不要使用 `latest`。
...
要运行指定 tag，在每次执行 compose 命令 (包括下面的升级命令) 之前先在 shell 中 export `OPEN_FLOW_IMAGE_TAG`: `export OPEN_FLOW_IMAGE_TAG=v0.1.0-beta.1`。仓库自带的 compose 文件默认使用 `latest`，而 `latest` 只会在首个稳定 Release 时出现；beta 阶段必须设置这个变量。
