...
| `<release-tag>` | конкретный Release, например `v0.3.0` (неизменяем) | вы разворачиваете production и хотите зафиксированную воспроизводимую сборку |
...
Для production фиксируйте тег Release, а не `latest`.
...
Чтобы запустить конкретный тег, задайте образ в override-файле или отредактируйте `image:` в `docker-compose.yml`.
