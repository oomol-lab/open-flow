...
- **데이터 의존성을 명시적으로 표현합니다.** 모든 Task와 Subflow는 이름과 타입이 있는 입력과 출력을 선언합니다. 각 에지는 특정 출력값을 특정 입력에 연결하므로 그래프가 런타임에서 사용하는 데이터 의존성 모델이 됩니다.
...
모든 Task와 Subflow는 이름과 타입이 있는 입력과 출력을 선언합니다. 에지는 특정 출력에서 특정 입력으로 값을 전달하며, 입력이 준비되면 런타임이 노드를 시작합니다.
